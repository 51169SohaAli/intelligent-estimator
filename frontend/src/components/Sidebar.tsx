...
export default function Sidebar({ isCollapsed = false }: SidebarProps) {
  return (
    <aside 
      // 🔑 Changed: Used fixed layout positioning with explicit z-index to lock it against the viewport edge
    ...
    >
      <div>
        {/* 🏢 Clean Brand Area: No hamburger clutter, just the logo mark */}
        <div className={`flex items-center mb-10 px-2 ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col animate-in fade-in duration-200">
              ...
              <p className="text-[10px] text-purple-300 font-semibold tracking-wider uppercase">Agile Suite</p>
            </div>
          )}
        </div>
      ...
      >
        ...
        {!isCollapsed && (
          <div className="overflow-hidden animate-in fade-in duration-200">
            <h4 className="text-xs font-bold text-white truncate">Soha Ali</h4>
            <p className="text-[10px] text-slate-400 truncate">Developer Account</p>
          </div>
        )}
